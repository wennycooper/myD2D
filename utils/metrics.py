...
def cal_pro_score(masks, amaps, max_step=200, expect_fpr=0.3):
    """
    Calculate PRO score for pixel-level anomaly detection evaluation
    
    Args:
        masks: Ground truth masks
        amaps: Predicted anomaly maps
        max_step: Maximum steps for threshold calculation
        expect_fpr: Expected false positive rate
...
def image_level_metrics(results, obj, metric):
    ...
    Args:
        results: Results dictionary containing ground truth and predictions
        obj: Object class name
        metric: Metric type ('image-auroc' or 'image-ap')
...
def pixel_level_metrics(results, obj, metric):
    ...
    Args:
        results: Results dictionary containing ground truth and predictions
        obj: Object class name
        metric: Metric type ('pixel-auroc' or 'pixel-aupro')
